...
import br.com.fiap.pacman.beans.*;
...
public class Game extends JFrame implements KeyListener {
	...
	private Booster booster = new Booster(400, 400, 20);
	...
	private int turnosDeInvensibilidade = 0;
	...
	private void run() {
		while (player.getLife() > 0) {
			if(player.isInvencivel()) turnosDeInvensibilidade--;
			if(turnosDeInvensibilidade == 0) player.setInvencivel(false);
			...
			if(player.colide(bomb)) player.perdeVida();

			if(player.colide(ghost1)) player.perdeVida();
			if(player.colide(ghost2)) player.perdeVida();
			if(player.colide(ghost3)) player.perdeVida();
			if(player.colide(ghost4)) player.perdeVida();

			if(player.colide(booster)){
				player.setInvencivel(true);
				turnosDeInvensibilidade = booster.getDuracao();
			} 
